...
async function start() {
  ...
  for (const pkg of PACKAGES) {
    ...
    const cachePath = run(
      `brew --cache --bottle-tag=arm64_sonoma ${pkg}`,
    ).trim();
  ...
  fs.copyFileSync(
    path.join(TEMP_DIR, "jpeg-xl", jpegXlDir, "bin/cjxl"),
    path.join(TARGET_DIR, "cjxl"),
  );
  fs.copyFileSync(
    path.join(TEMP_DIR, "jpeg-xl", jpegXlDir, "bin/djxl"),
    path.join(TARGET_DIR, "djxl"),
  );
  run(
    `chmod +x "${path.join(TARGET_DIR, "cjxl")}" "${path.join(TARGET_DIR, "djxl")}"`,
  );
  ...
  for (const pkg of packagesExtracted) {
    ...
    if (fs.existsSync(libDir)) {
      ...
      for (const f of files) {
        if (
          f.endsWith(".dylib") &&
          !fs.lstatSync(path.join(libDir, f)).isSymbolicLink()
        ) {
  ...
  function patchBinary(binaryPath: string) {
    ...
    const lines = otoolOut
      .split("\n")
      .filter(
        (l) => l.includes("/opt/homebrew") || l.includes("@@HOMEBREW_PREFIX@@"),
      );
    ...
    for (const line of lines) {
      ...
      if (match) {
        ...
        if (binaryPath.endsWith(libName)) {
          run(
            `install_name_tool -id "@executable_path/libs/${libName}" "${binaryPath}"`,
          );
        } else {
          run(
            `install_name_tool -change "${oldPath}" "@executable_path/libs/${libName}" "${binaryPath}"`,
          );
        }
      }
    }
